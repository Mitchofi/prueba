...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM activationlink WHERE challenge = ?', (number, utils.U_UNCONFIRMED)
            ).fetchone()
...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            db.execute(
                'INSERT INTO activationlink (id, challenge, state, username, password, salt, email) VALUES (NULL, ?, ?, ?, ?, ?, ?)',
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                db.execute(
                    'UPDATE forgotlink SET salt = ? WHERE id = ?',
